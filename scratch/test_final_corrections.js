...
// Import or recreate the functions to test exactly what train.js produces
...
async function run() {
  ...
  const mlPayload = {
    ...
    destination: {
      ...
      scheduled_arrival: destStop.scheduledArrivalTime,
    },
    current_location: {
  ...
  const nad = live.route.find((s) => s.stationCode === 'NAD');
  ...
  futureMainCodes.forEach((code) => {
    const stop = live.route.find((s) => s.stationCode === code);
  ...
  const nextMain = live.route.find((s) => s.stationCode === 'KOTA');
